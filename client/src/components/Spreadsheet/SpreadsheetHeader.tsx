...
const fetchSpreadsheetName = async (spreadsheetId: number): Promise<string> => {
    ...
    if (!response.ok) {
        throw new Error('Failed to fetch spreadsheet name');
    }
...
const fetchSpreadsheetType = async (spreadsheetId: number): Promise<string> => {
    ...
    if (!response.ok) {
        throw new Error('Failed to fetch spreadsheet type');
    }

    return response.json();
};

const SpreadsheetHeader: React.FC<SpreadsheetProps> = ({ uid, spreadsheetId, saving, setSaving }) => {
